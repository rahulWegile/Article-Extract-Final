# Grouping prompt for Gujarati.
#
# Deliberately a copy of Hindi's grouping prompt (the variant with
# the WHITESPACE GAPS and MANDATORY COVERAGE AUDIT sections), not
# the default one the other non-Hindi languages use. Gujarati is
# routed through OpenAI, not Gemini -- only the PROMPT is shared
# with Hindi, not the provider.
#
...
null means there is no neighbour on that side, OR that the
space between them contains some other block (e.g. a photo
of a different width) and the distance could not be measured
as clean whitespace -- treat null as "no gap signal available
here", not as evidence either way.
...
- a large gap_above on a title block
      -> a strong signal of a new article. Weigh it together
         with the other cues below (headline ownership, ruled
         lines, the image) rather than on its own -- do not
         attach that title to the story above it unless those
         other cues clearly say it belongs there.
...
reliable separation cues available to you -- but it is one
signal among several, not a replacement for them. Weigh it
together with raw proximity, headline ownership, and the
image itself, and never let it alone override what those
other cues show.
...
- treat it as strong visual evidence that the adjacent regions are
  separated editorial units
- do NOT merge the regions merely because their bounding boxes are
  close or touching
- use the rule together with headline ownership, column flow, and
  semantic continuity

A TRUE rule-line signal is stronger than physical proximity.

However, a rule line alone does NOT automatically create a new article.
If the content on both sides is clearly one story, keep it together.

When a rule line separates two independent headline/body regions,
treat them as separate article candidates.

Do NOT treat the rule itself as an article block.
It is only a layout-separation signal.
